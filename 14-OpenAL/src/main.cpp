...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	//Ada
	modelAda.loadModel("../Modelos/Animations/Pistol Kneeling Idle.dae");
	modelAda.setShader(&shaderMulLighting);
...
void renderScene(bool renderParticles){
	...
	//modelMatrixAda[3][1] -= 30.0f;
	modelMatrixAda = glm::scale(modelMatrixAda, glm::vec3(0.05f));
	modelMatrixAda = glm::rotate(modelMatrixAda, (float)glfwGetTime() * 0.5f, glm::vec3(0.2f, 1.0f, 0.35f));

	glDisable(GL_CULL_FACE);

	static bool once = false;
	static std::vector<std::vector<glm::vec4>> vertexes;
	static std::vector<glm::vec4> vertexesJoined;

	if (!once)
	{
		vertexesJoined = modelAda.render(modelMatrixAda, ssboVertexWorldSpace, 8);
		//vertexesJoined = std::vector<glm::vec4>(vertexesJoined.begin() + 14450, vertexesJoined.begin() + 14500);
		vertexesJoined = std::vector<glm::vec4>(vertexesJoined.begin() + 14494, vertexesJoined.begin() + 14495);
		//vertexesJoined = std::vector<glm::vec4>(vertexesJoined.begin() + 14485, vertexesJoined.begin() + 14486);
		//vertexes = mayowModelAnimate.render(modelMatrixMayowBody, ssboVertexWorldSpace);
	}
	else
	{
		modelAda.render(modelMatrixAda);
		//mayowModelAnimate.render(modelMatrixMayowBody);
	}
	modelAda.render(modelMatrixAda);

	//std::cout << vertexesJoined.size() << "\n";

	static int frames = 0;

	if (frames == vertexesJoined.size()*90)
		frames = 0;

	glm::mat4 modelMatrixEsfera(1.0f);
	//modelMatrixLinterna[3][1] = terrain.getHeightTerrain(modelMatrixLinterna[3][0], modelMatrixLinterna[3][2]);
	
	//std::cout << glm::vec3(vertexesJoined[frames]).x << " " << glm::vec3(vertexesJoined[frames]).y << " " << glm::vec3(vertexesJoined[frames]).z << " " << "\n";
	//modelMatrixLinterna = glm::translate(modelMatrixLinterna, glm::vec3(vertexesJoined[vertexesJoined.size() - frames - 1]));
	modelMatrixEsfera = glm::translate(modelMatrixEsfera, glm::vec3(vertexesJoined[frames/90]));

	modelMatrixEsfera = glm::scale(modelMatrixEsfera, glm::vec3(0.2f));

	modelEsfera.render(modelMatrixEsfera);

	/*glm::mat4 modelMatrixLinterna(1.0f);
	modelMatrixLinterna = glm::translate(modelMatrixLinterna, glm::vec3(vertexesJoined[0]) + glm::vec3(0.0f, 4.0f, 0.0f));
	modelMatrixLinterna = glm::scale(modelMatrixLinterna, glm::vec3(1.8f));*/
	glm::mat4 modelMatrixLinterna(modelMatrixAda);
	modelMatrixLinterna[3] = glm::vec4(glm::vec3(vertexesJoined[0]), modelMatrixLinterna[3][3]);
	modelMatrixLinterna = glm::scale(modelMatrixLinterna, glm::vec3(3.5f));
	modelMatrixLinterna = glm::translate(modelMatrixLinterna, glm::vec3(0.5f, 2.0f, 1.5f));
