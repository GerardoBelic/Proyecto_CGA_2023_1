...
uniform float porcentaje;

vec3 calculateDirectionalLight(Light light, vec3 direction){

	vec3 colorTex1 = vec3(texture(texture1, our_uv));
	vec3 colorTex2 = vec3(texture(texture2, our_uv));
	vec3 color = mix(colorTex1, colorTex2, porcentaje);
