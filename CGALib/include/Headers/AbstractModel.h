...
public:
	...
	void render(int indexInit, int indexSize, glm::mat4 parentTrans = glm::mat4(1.0f)) {
		...
		/*if (!recording)
			...
			lastMatrixes.push_back(modelMatrix);*/
		/*if (recording)
		{
			lastMatrixes.push_back(modelMatrix);
			lastVertexGroups.push_back(vertexArray.size());
		}*/
	}
	...
	/*static std::vector<glm::mat4> lastMatrixes;
	static std::vector<unsigned> lastVertexGroups;
